<x-layouts.app>
    <x-slot:title>Edit Kendaraan</x-slot:title>
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-semibold text-slate-900">Edit Kendaraan</h1>
        <p class="mt-1 text-sm text-slate-500">Perbarui informasi {{ $kendaraan->plate_number }}</p>
    </div>

    <div class="max-w-2xl">
        <x-card>
            <form method="POST" action="{{ route('kendaraan.update', $kendaraan) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="brand" id="brand" value="{{ old('brand', $kendaraan->brand) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="model" id="model" value="{{ old('model', $kendaraan->model) }}" required
                               class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition">
                        @error('model')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <input type="text" name="plate_number" id="plate_number" value="{{ old('plate_number', $kendaraan->plate_number) }}" required
                           class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition">
                    @error('plate_number')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" id="type" required
                            ...
                            @foreach ($types as $t)
                                <option value="{{ $t->value }}" {{ old('type', $kendaraan->type->value) === $t->value ? 'selected' : '' }}>{{ $t->label() }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                                class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition">
                            <option value="ACTIVE" {{ old('status', $kendaraan->status) === 'ACTIVE' ? 'selected' : '' }}>Aktif</option>
                            <option value="INACTIVE" {{ old('status', $kendaraan->status) === 'INACTIVE' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition resize-none">{{ old('notes', $kendaraan->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
