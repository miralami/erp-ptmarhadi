<x-layouts.app>
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-semibold text-slate-900">Edit Invoice</h1>
        <p class="mt-1 text-sm text-slate-500">Perbarui informasi invoice {{ $invoice->invoice_number }}</p>
    </div>
            ...
            <form method="POST" action="{{ route('invoices.update', $invoice) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full px-4 py-2.5 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition resize-none">{{ old('notes', $invoice->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
